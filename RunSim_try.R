...
sim_one <- function(i,
  ...
  ss_s2 <- ImputeQuality(Y = dat$Y_miss, S = dat$S, A = dat$A, basis = "Poly(S)")
  ...
  ss_sw2 <- ImputeQuality(Y = dat$Y_miss, S = dat$S, A = dat$A, X = dat %>% select(contains("W")), basis = "Poly(S) + X")
  ...
  #   basis = "Poly(S) + X"
  # )
  ss_sw_int2 <- ImputeQuality(Y = dat$Y_miss, S = dat$S, A = dat$A, X = add_X_all, basis = "Poly(S) + X")
